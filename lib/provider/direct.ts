...
/**
 * One request to Instagram's unauthenticated web_profile_info endpoint.
 *
 ...
 */
async function attemptOnce(username: string, dispatcher?: Dispatcher): Promise<FollowerData | null> {
...
/**
 * Fetch follower data, retrying a few times. Instagram intermittently rejects
 * requests from datacenter IPs; a retry (often landing on a different Vercel
 * egress IP) usually succeeds. `dispatcher` is an optional undici Agent (the
 * proxy provider passes a ProxyAgent).
 */
export async function fetchWebProfile(
  ...
  for (let i = 0; i < ATTEMPTS; i++) {
    const result = await attemptOnce(username, dispatcher);
